...
•       **Apple Push Notification Service (APNs)** for delivering proximity alerts, friend requests, chat messages, range exit warnings, blackout zone suggestions, precise location share alerts, event alerts, and blocked-user proximity warnings to iOS devices. APNs is the only mechanism Apple provides for push to iOS, so this is a requirement rather than a choice.
...
•       **Sign in with Apple / Sign in with Google (OAuth providers)** for third-party account creation and authentication during onboarding. The client completes the OAuth flow with the respective provider and passes the resulting identity token to the API Server, which validates it and issues a Squad Seeker JWT. All subsequent authentication uses that JWT; the OAuth providers are not contacted again after onboarding.

•       **External content-analysis service** (e.g., AWS Rekognition or Google Vision) for automated profile photo moderation. Called by the API Server's Moderation module; the service returns a flag/clean verdict that determines whether a submitted photo is saved to the user's profile or held in the human review queue.

## **1.2 Connectors**

Communication between components uses the following connectors:

**Client ↔ API Server: HTTPS REST.** Most client-server interactions use standard HTTPS REST requests with JSON payloads. JSON Web Tokens (JWTs) carry authentication. Interactions include: login and account creation (including OAuth token exchange), posting bios, subscribing to and unsubscribing from interests, sending and accepting friend requests, defining and removing blackout zones, fetching nearby users for a given interest, initiating and cancelling precise location shares, managing circles, RSVP'ing to and creating events, and interest catalog browsing.
...
**API Server ↔ Location Service: internal HTTPS.** The two backend services exchange information over internal HTTPS calls. For example, the API Server queries the Location Service when it needs the current set of nearby users for a given interest in order to render the map view. The API Server also notifies the Location Service when a precise location share is created or cancelled (`POST /internal/location-share`), so the Location Service can update its access table and serve exact or fuzzy coordinates to recipients accordingly. The API Server similarly notifies the Location Service when a new event is created or expires, so the event pin appears in and is removed from the spatial index correctly.
...
| Sign in with Apple / Google | Apple's / Google's servers (external) |
| Content-analysis service | Third-party cloud (external) |
...
Most client-server interaction is request-response: log in, fetch a profile, subscribe to an interest, send a friend request, define a blackout zone, fetch the list of nearby users for a given interest, initiate or cancel a precise location share, manage circles, create or RSVP to an event, and browse the interest catalog. These all fit cleanly into the REST protocol: each request is independent, each has a clear resource being acted on, and each completes with a single response.

| \# | Data Communicated | Example |
| :---- | :---- | :---- |
| 1 | User input | interest selections, profile data, friend-request targets, blackout zone coordinates, precise share recipients and duration, event title/time/location, circle membership changes |
| 2 | Result | success acknowledgments, lists of nearby users, profile data, interest catalog, active share summaries, event detail, error messages |
...
**OAuth token exchange.** For Sign in with Apple and Sign in with Google, the client completes the OAuth flow with the respective provider client-side and receives a provider-issued identity token. The client sends this token to the API Server (`POST /auth/oauth`), which validates it against the provider's published public keys, creates or retrieves the corresponding user account, and issues a Squad Seeker JWT in response. If the provider email matches an existing email/password account, the accounts are merged at this step and the JWT is issued for the merged account. From this point the JWT flow is identical to email/password login; the OAuth provider is not contacted again during the session.
...
Push notifications are dispatched from the API Server to Apple's Push Notification Service over HTTPS. Each iOS client registers a device token at login that the server stores; when a notification needs to be delivered, the server looks up the token and posts a JSON payload to APNs, which delivers it to the device.

The following notification types are dispatched via this connector: proximity alerts, friend requests, chat messages, range exit warnings (geofenced chat suspension), blackout zone suggestions, precise location share alerts (share started for recipient; share expired for sharer), event alerts (new nearby event, starting soon for RSVP'd users, private event invitation), and blocked user proximity warnings.
...
| 6 | Location Service | updateGeoIndex() then computeProximityMatches(); updates the user's position, then queries the spatial index for other users subscribed to the same interest within the per-interest range cap. Before returning match results, checks the active precise-share access table to determine whether to return exact or fuzzy coordinates for each matched mutual friend. |  \- |
...
## **5.7 Precise Location Time-Window Sharing**

**Conceptual flow.** A user initiates a precise location share with one or more mutual friends or a named circle for a fixed duration. The API Server persists the share record, registers it with the Location Service so the recipient sees exact coordinates rather than fuzzy ones, and notifies recipients via APNs. When the share expires or is cancelled early, the Location Service removes the access record and the sharer is notified.

| Step | Component | Function | Data Across Connector |
| :---- | :---- | :---- | :---- |
| 1 | iOS Client → API Server | HTTPS POST /location/share | { sharerId, recipientIds[], circleIds[], durationSeconds } |
| 2 | API Server | createLocationShare(); persists share record in PostgreSQL with expiresAt timestamp; expands any circleIds to individual recipientIds. |  \- |
| 3 | API Server → Location Service | Internal HTTPS POST /internal/location-share | { shareId, sharerId, recipientIds[], expiresAt }; Location Service registers the share in its in-memory access table |
| 4 | API Server → APNs → iOS Client (each recipient) | APNs payload | "[Username] is sharing their precise location with you for [duration]" |
| 5 | iOS Client (recipient) | Map renders sharer's pin | On next location update from sharer, Location Service returns exact coordinates to authorized recipients rather than fuzzy centroid |
| 6 | Location Service | checkShareExpiry() (periodic background task); detects shares whose expiresAt has passed and removes them from the access table. |  \- |
| 7 | Location Service → API Server | Internal HTTPS POST /internal/location-share-expired | { shareId, sharerId, recipientIds[] } |
| 8 | API Server → APNs → iOS Client (sharer only) | APNs payload | "Your precise location share with [name or circle] has ended"; multiple simultaneous expiries are consolidated into one notification |
| 9 | iOS Client (sharer, early cancel) → API Server | HTTPS DELETE /location/share/{shareId} | { shareId } |
| 10 | API Server → Location Service | Internal HTTPS DELETE /internal/location-share/{shareId} | Location Service removes share from access table immediately; recipient's view of sharer's pin reverts silently on next map refresh |

## **5.8 Onboarding**

**Conceptual flow.** A new user creates an account (via OAuth or email/password), selects a username, chooses at least one interest, and grants location and notification permissions. The backend issues a JWT after account creation and registers a device token for APNs after notification permission is granted. If location permission is denied, the client proceeds to the map in a degraded state without sending location updates.

| Step | Component | Function | Data Across Connector |
| :---- | :---- | :---- | :---- |
| 1 | iOS Client | OAuth flow completes client-side (Sign in with Apple or Google); or user enters email/password. No network call to API Server yet. |  \- |
| 2 | iOS Client → API Server | HTTPS POST /auth/oauth or POST /auth/register | OAuth: { provider, identityToken }; Email: { email, password } |
| 3 | API Server | createOrRetrieveAccount(); validates OAuth token against provider public keys, or hashes and stores password; merges accounts if OAuth email matches existing email/password account; issues JWT. |  \- |
| 4 | API Server → iOS Client | HTTPS response | { jwt, userId, isNewUser } |
| 5 | iOS Client → API Server | HTTPS POST /users/username | { username } |
| 6 | API Server | reserveUsername(); checks uniqueness in PostgreSQL; returns conflict error if taken. |  \- |
| 7 | iOS Client → API Server | HTTPS POST /interests/subscribe | { interestIds[], visibility: "friends_only" } — batch subscription with onboarding defaults |
| 8 | iOS Client (CoreLocation) | requestAlwaysAuthorization(); iOS system dialog fires; result stored locally. If denied, client proceeds without sending location updates. |  \- |
| 9 | iOS Client (UNUserNotificationCenter) | requestAuthorization(); iOS system dialog fires; result stored locally. |  \- |
| 10 | iOS Client → API Server | HTTPS POST /devices/token | { apnsDeviceToken }; sent only if notification permission was granted; omitted if denied |
| 11 | iOS Client | Map screen loads; if location permission granted, location updates begin (see 5.5); if denied, degraded banner state shown. |  \- |

## **5.9 Interest Discovery & Management**

**Conceptual flow.** A user opens the Interests tab. The client fetches the Popular Near You section (which requires the Location Service to calculate local activity levels) and the full interest catalog (a pure database read). The user subscribes to a new interest or unsubscribes from an existing one; unsubscription triggers a cache invalidation in the Location Service so the user immediately disappears from that interest's discoverable pool.

| Step | Component | Function | Data Across Connector |
| :---- | :---- | :---- | :---- |
| 1 | iOS Client → API Server | HTTPS GET /interests/popular?lat=&lng= | Current user position (from last known GPS fix) |
| 2 | API Server → Location Service | Internal HTTPS GET /internal/interests/active?lat=&lng=&radius= | Position and radius; Location Service queries its geospatial index for active subscriber counts per interest near that position |
| 3 | Location Service → API Server | Internal response | { interests: [{ interestId, activityLevel: "high" \| "moderate" \| "low" }] }; relative labels derived from subscriber count thresholds |
| 4 | API Server → iOS Client | HTTPS response | Combined payload: { popularNearYou: [...], myInterests: [...] } |
| 5 | iOS Client → API Server | HTTPS GET /interests/categories | Full catalog browse — pure database read; no Location Service involvement |
| 6 | API Server → iOS Client | HTTPS response | { categories: [{ name, interests: [...] }] } |
| 7 | iOS Client → API Server | HTTPS POST /interests/subscribe | { interestId, visibility, notificationMode } |
| 8 | API Server | saveSubscription(); persists in PostgreSQL; no Location Service notification needed — Location Service picks up the new subscriber on their next GPS update. |  \- |
| 9 | iOS Client → API Server | HTTPS DELETE /interests/{interestId}/subscribe |  \- |
| 10 | API Server → Location Service | Internal HTTPS POST /internal/interests/unsubscribe | { userId, interestId }; Location Service removes user from that interest's active index immediately |

## **5.10 Events / Activity Marker**

**Conceptual flow.** A user long-presses a location on the map and creates an event. The API Server validates the rate limit, persists the event, and registers it with the Location Service so it appears as a pin on nearby users' maps. Nearby interest subscribers receive an APNs notification. A user who sees the event may RSVP, gaining access to the event chat. The event auto-expires 2 hours after its scheduled start time, at which point the Location Service removes the pin from the spatial index and the event chat converts to a persistent group chat.

| Step | Component | Function | Data Across Connector |
| :---- | :---- | :---- | :---- |
| 1 | iOS Client | Long-press on map → context menu → "Create Event here"; event creation form pre-populated with tapped coordinates. |  \- |
| 2 | iOS Client → API Server | HTTPS POST /events | { title, scheduledAt, lat, lng, interestId, visibility: "public" \| "private" } |
| 3 | API Server | createEvent(); checks rate limit (≤3 events per 7-day rolling window per user); rejects with informative error if exceeded; persists event record in PostgreSQL with expiresAt = scheduledAt + 2 hours. |  \- |
| 4 | API Server → Location Service | Internal HTTPS POST /internal/events | { eventId, lat, lng, interestId, expiresAt }; Location Service adds event to its spatial index |
| 5 | API Server → APNs → iOS Client (nearby interest subscribers) | APNs payload | "New [interest] event near you: [title] at [time]" |
| 6 | iOS Client (nearby user) → API Server | HTTPS POST /events/{id}/rsvp |  \- |
| 7 | API Server | recordRsvp(); adds user to event attendees in PostgreSQL; adds user to event chat session. |  \- |
| 8 | API Server → iOS Client | HTTPS response | { eventChatSessionId } — client may now open the event chat WebSocket |
| 9 | Location Service | checkEventExpiry() (periodic background task, same scheduler as checkShareExpiry); detects events whose expiresAt has passed. |  \- |
| 10 | Location Service → API Server | Internal HTTPS POST /internal/events/expired | { eventId } |
| 11 | API Server | expireEvent(); removes event pin from PostgreSQL active events; converts event chat to a persistent group chat — session remains accessible to all attendees at time of expiry; no new users may join. |  \- |
| 12 | API Server → APNs → iOS Client (RSVP'd users) | APNs payload | "[Event title] starts in 30 minutes" — fired at scheduledAt - 30 minutes, independently of the expiry path |
